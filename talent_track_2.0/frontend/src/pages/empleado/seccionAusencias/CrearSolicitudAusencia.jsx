...
export default function CrearSolicitudAusencia() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Si falla la carga inicial, mostramos error pero permitimos reintentar si se recarga
        console.error(e);
      } finally {
        setLoading(false);
      }
  ...
  const closeModal = () => {
      ...
      // Si fue éxito, redirigimos al cerrar
  ...
  const onSubmit = async (e) => {
    ...
    if (form.fecha_fin && new Date(form.fecha_fin) < new Date(form.fecha_inicio)) {
      return showModal('error', 'Fechas inválidas', "La fecha de fin no puede ser anterior al inicio.");
    }
    ...
    try {
      ...
      // ÉXITO
      showModal('success', '¡Solicitud Enviada!', 'Tu solicitud ha sido registrada correctamente y está pendiente de aprobación.');
      
    } catch (e2) {
      // ERROR
      showModal('error', 'Error al guardar', e2?.message || "No se pudo crear la solicitud.");
    }
  };
    ...
    <div className="layout layout-watermark" style={layoutWrapperStyle}>
      ...
      <main style={mainAreaStyle}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
                <h1 style={{ fontSize: '1.8rem', fontWeight: '800', color: '#1e293b', margin: 0 }}>Nueva Solicitud</h1>
                <p style={{ color: '#64748b', marginTop: '4px', fontSize: '1rem' }}>Registra una ausencia programada.</p>
            </div>
            ...
            <div style={{ display: 'flex', gap: '30px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: '1', minWidth: '300px', backgroundColor: '#fff', borderRadius: '16px', padding: '25px', border: '1px solid #e2e8f0' }}>
                    ...
                    <div style={{ marginBottom: '15px', paddingBottom: '15px', borderBottom: '1px solid #f1f5f9' }}>
                        ...
                        <p style={{ color: '#64748b', fontSize: '0.9rem', margin: 0 }}>Vista previa de tu solicitud.</p>
                    </div>
                    ...
                    <div style={{ backgroundColor: '#fff1f2', padding: '15px', borderRadius: '8px', border: '1px solid #fecdd3', marginTop: '20px' }}>
                        <div style={{ display: 'flex', gap: '10px' }}>
                            <i className='bx bx-info-circle' style={{ color: '#e11d48', fontSize: '1.2rem' }}></i>
                            <p style={{ fontSize: '0.85rem', color: '#881337', margin: 0, lineHeight: '1.4' }}>
                                Verifica los días festivos o fines de semana en tu calendario local, ya que aquí se muestran días calendario.
                            </p>
                        </div>
                    </div>
