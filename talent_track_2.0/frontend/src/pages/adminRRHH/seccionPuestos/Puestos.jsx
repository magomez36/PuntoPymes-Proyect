...
export default function Puestos() {
  ...
  const eliminar = async (id) => {
    ...
    try {
      const res = await apiFetch(`/api/rrhh/puestos/${id}/`, { method: "DELETE" });
    ...
    <div className="layout">
      ...
      <main className="main-content">
        ...
        <div style={{ marginBottom: 12 }}>
          <button onClick={() => navigate("/rrhh/puestos/crear")}>Crear Puesto</button>
        </div>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id}>
                  ...
                  <td>
                    <button onClick={() => navigate(`/rrhh/puestos/editar/${r.id}`)}>Editar</button>{" "}
              ...
              {rows.length === 0 && (
                <tr>
                  <td colSpan="6">No hay puestos registrados.</td>
                </tr>
              )}
            </tbody>
          </table>
