...
export default function CrearRolEmp() {
  ...
  // --- ESTILOS ---
  const inputStyle = { width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #d1d5db', outline: 'none', fontSize: '0.9rem', backgroundColor: '#fff', color: '#334155' };
      ...
      <main className="main-content" style={{ flex: 1, padding: '30px 30px 30px 110px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
            <div>
                ...
                <p style={{ color: '#64748b', marginTop: '4px', fontSize: '1rem' }}>Define los permisos y accesos.</p>
            </div>
        ...
        {loading ? <p style={{textAlign:'center', color:'#64748b'}}>Cargando...</p> : (
            // AQUI CAMBIE EL GRID POR FLEX PARA CONTROLAR MEJOR EL ANCHO
            <div style={{ display: 'flex', gap: '30px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                
                {/* COLUMNA 1: FORMULARIO (Se expande) */}
                ...
                {/* COLUMNA 2: GUÍA (Ancho fijo medio) */}
                <div style={{ flex: '0 0 320px', backgroundColor: '#f8fafc', borderRadius: '16px', border: '1px dashed #cbd5e1', padding: '25px' }}>
                    <h3 style={{ margin: '0 0 20px 0', color: '#1e293b', display: 'flex', alignItems: 'center', gap: '10px', fontSize: '1rem' }}>
                        <i className='bx bx-shield-quarter' style={{ color: '#dc2626', fontSize:'1.2rem' }}></i> Niveles de Acceso
                    </h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        {[
                            { role: 'Super Admin', desc: 'Control total.', icon: 'bx-crown' },
                            { role: 'RRHH', desc: 'Gestión de personal.', icon: 'bx-user-pin' },
                            { role: 'Manager', desc: 'Supervisión de equipos.', icon: 'bx-briefcase' },
                            { role: 'Empleado', desc: 'Acceso básico.', icon: 'bx-user' }
                        ].map((item, i) => (
                            <div key={i} style={{ display: 'flex', gap: '12px' }}>
                                <div style={{ minWidth: '32px', height: '32px', borderRadius: '8px', background: '#fff', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748b' }}>
                                    <i className={`bx ${item.icon}`}></i>
                                </div>
                                <div>
                                    <strong style={{ display: 'block', color: '#334155', fontSize: '0.85rem' }}>{item.role}</strong>
        ...
        {modal.show && (
            <div className="modal-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(2px)' }}>
                <div style={{ background: 'white', padding: '32px', borderRadius: '16px', width: '90%', maxWidth: '400px', textAlign: 'center' }}>
