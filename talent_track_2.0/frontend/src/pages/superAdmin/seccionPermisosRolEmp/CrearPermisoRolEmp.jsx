...
export default function CrearPermisoRolEmp() {
  ...
  const loadRoles = async (empresaId) => {
    ...
    try {
      const res = await apiFetch(`${API_BASE}/helpers/roles-por-empresa/?empresa_id=${empresaId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const onChange = async (e) => {
    ...
    if (name === "empresa_id") await loadRoles(value);
  };
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const res = await apiFetch(`${API_BASE}/permisos/`, {
        method: "POST",
      ...
      alert(e.message || "Error creando permiso.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="layout" style={{ display: 'flex', minHeight: '100vh', background: '#f8fafc' }}>
      ...
      <main className="main-content" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        {/* === HEADER === */}
        {/* CAMBIO: Eliminado el 'margin: 0 auto' y 'maxWidth'. Ahora ocupa el 100% y el padding alinea el texto a la izquierda */}
        <header style={{ width: '100%', backgroundColor: 'white', padding: '16px 32px', borderBottom: '1px solid #e2e8f0' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'8px', fontSize:'0.85rem', color:'#64748b' }}>
                ...
                <span style={{color:'#dc2626', fontWeight:'600'}}>Nuevo Permiso</span>
            </div>
        </header>

        {/* === CONTENIDO PRINCIPAL === */}
        {/* MANTENIDO: Este contenedor SÍ se mantiene centrado con maxWidth 1100px */}
        <div className="content-area" style={{ maxWidth: '1100px', margin: '0 auto', padding: '40px 20px', width: '100%' }}>
            
            {/* TÍTULO HERO (Alineado a la izquierda DE ESTE contenedor centrado) */}
            <div style={{ marginBottom: '32px', textAlign: 'left' }}>
                <h1 style={{ fontSize: '1.8rem', fontWeight: '800', color: '#1e293b', margin: '0 0 8px 0', letterSpacing:'-0.5px' }}>
                    Crear Nuevo Permiso
                </h1>
                <p style={{ color: '#64748b', margin: 0, fontSize:'0.95rem' }}>
                    Define un nuevo código de acceso y asígnalo a un rol específico.
                </p>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 350px', gap: '32px', alignItems: 'start' }}>
                            ...
                            <div>
                                ...
                                <div style={{ position: 'relative' }}>
                                    ...
                                    <textarea 
                                        ...
                                        onChange={onChange} 
                                        rows="3" 
                                        placeholder="Explica qué permite hacer este permiso..." 
                ...
                <div style={{ background: 'white', borderRadius: '16px', boxShadow: '0 4px 20px -5px rgba(0,0,0,0.05)', border: '1px solid #e2e8f0', padding: '24px' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'16px' }}>
                        ...
                        <h3 style={{ fontSize: '1rem', fontWeight: '700', color: '#1e293b', margin:0, textAlign: 'left' }}>Ayuda Rápida</h3>
                    </div>
                    
                    <div style={{ fontSize: '0.85rem', color: '#64748b', display:'flex', flexDirection:'column', gap:'16px', textAlign: 'left' }}>
                        <p style={{ margin: 0 }}>
                            <strong>Formato de Código:</strong><br/>
                            Se recomienda usar mayúsculas y guiones bajos para mantener la consistencia técnica.
                            <br/>
                            <code style={{ background:'#f1f5f9', padding:'2px 6px', borderRadius:'4px', color:'#dc2626', marginTop:'4px', display:'inline-block' }}>EDITAR_USUARIOS</code>
                        </p>
                        
                        <p style={{ margin: 0 }}>
                            <strong>Jerarquía:</strong><br/>
                            Los permisos se asignan a un <em>Rol</em>, y los roles se asignan a los <em>Empleados</em>.
                        </p>
                        ...
                        <div style={{ background:'#fffbeb', padding:'12px', borderRadius:'8px', border:'1px solid #fcd34d', color:'#b45309' }}>
                            ...
                            Los cambios en permisos pueden requerir que los usuarios vuelvan a iniciar sesión.
                        </div>
                    </div>
                </div>
        ...
        {showSuccessModal && (
            <div className="modal-overlay" style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(2px)' }}>
                <div className="modal-content" style={{ backgroundColor: 'white', padding: '32px', borderRadius: '16px', width: '90%', maxWidth: '400px', textAlign: 'center', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
                    ...
                    <h3 style={{ fontSize: '1.5rem', fontWeight: '700', color: '#111827', marginBottom: '12px' }}>¡Permiso Creado!</h3>
                    <p style={{ color: '#6b7280', marginBottom: '24px' }}>El permiso ha sido asignado correctamente al rol.</p>
