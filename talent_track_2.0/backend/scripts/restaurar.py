...
respaldo_json = os.path.join(os.path.dirname(os.path.abspath(__file__)), "respaldo_completo.json")
...
for modelo, nombre_tabla in tablas:
    ...
    for registro in registros:
        # Eliminar '_state' si existe
        registro.pop("_state", None)

        # Convertir strings de fecha a datetime si existe 'creada_el' o 'ultimo_acceso'
        for campo in ['creada_el', 'ultimo_acceso']:
            if campo in registro and registro[campo]:
                registro[campo] = datetime.fromisoformat(registro[campo].replace("Z", "+00:00"))
